...
def fileModifier(dataFile):
    try:
        config = cnfloader.load_properties()
        if not os.path.isfile(dataFile):
            raise ValueError((400, f"Missing data file at: {dataFile}"))
        
        keepColumns = config.get("keepOnlyColumnsFromSource", "")
...
def fileRename(dataFile, renameFrom, renameTo):
    try:
...
def dataWriter(dataFile, outputDirectory, FileName, insertColoumns):
    try:
        if not os.path.isfile(dataFile):
